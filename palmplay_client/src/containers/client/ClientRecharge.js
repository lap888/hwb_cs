 ...
 * @Last Modified time: 2019-04-25 15:43:53
 */

import { Table, Card, Col, Row } from 'antd';
import { Send } from 'HttpPost';

export default class ClientRecharge extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: true,
    ...
    onChangeStatus (status) {
        this.setState({ status }, () => this.fetchMeiningList(1));
    }
            ...
            <div style={{ background: '#ECECEC', padding: '26px' }}>
                <Row gutter={24}>
                    <Col span={6} onClick={() => this.onChangeStatus(1)}>
                        <Card title="运行矿机" bordered={false} hoverable>
                    ...
                    <Col span={6} onClick={() => this.onChangeStatus(0)}>
                        <Card title="历史矿机" bordered={false} hoverable>
                            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Col span={6}>
                        <Card title="累计收取钻石" bordered={false} hoverable>
                            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                <p style={{ fontSize: 26 }}>{`${parseInt(collectNumber)}`}</p>
        ...
        const columns = [
            {
                ...
                width: '25%',
                render: (text, record) => (
                    <p style={{ margin: 0, color: MINING_TYPE_LIST[record['type']]['color'] }}>{`${MINING_TYPE_LIST[record['type']]['minning_name']}`}</p>
                )
            },{
                title: '开始时间',
                dataIndex: 'beginTime',
